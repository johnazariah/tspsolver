...
module TSPParser =
            ...
            let copyExplicitEdgeWeights (problem : Problem) (g : WeightedCompleteGraph) : WeightedCompleteGraph option =
                ...
                let copyFullWeights (lines : WeightLine list) =
                    let copySingleLine (g : WeightedCompleteGraph, y : int) (wl : WeightLine) =
                        wl.unapply
                        |> List.fold (fun x w -> g[NodeId x, NodeId y] <- w;  x + 1) 0
                ...
                let computeEdgeWeight (weight_func : ('n * 'n -> int) option) (unapply : 'n -> int) (nodes : 'n list) =
                    option {
                        ...
                        for p1 in 1 .. dim do
                            for p2 in 1 .. dim do
                                let d = f (m[p1], m[p2])
                                g[NodeId (p2 - 1), NodeId (p1 - 1)] <- Weight d
                        return g
                    }

                problem.NodeCoordSection
                |> Option.bind (function
                    | NodeCoordSection.Nodes2D nodes -> computeEdgeWeight f2 (fun (n : NodeCoord2D) -> n.NodeId.unapply) nodes
                    | NodeCoordSection.Nodes3D nodes -> computeEdgeWeight f3 (fun (n : NodeCoord3D) -> n.NodeId.unapply) nodes)
    ...
    let parseTextToProblem str =
        match run problem str with
